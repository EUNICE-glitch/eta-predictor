...
class ETARequest(BaseModel): 
    ...
    origin_lon:      float = Field(..., ge=-180,  le=180,                               description='Origin longitude (-180 to 180)') 
    ...
    hour_of_day:     int   = Field(..., ge=0,     le=23, 
                                  description='Departure hour (0=midnight, 
23=11pm)') 
    day_of_week:     int   = Field(..., ge=0,     le=6, 
                                  description='Departure day (0=Monday, 6=Sunday)') 
    num_stops:       int   = Field(1, ge=1, le=20, 
                                  description='Number of delivery stops (default: 
1)') 
    ...
    @field_validator('origin_lat', 'origin_lon', 'dest_lat', 'dest_lon', 
mode='before') 
    @classmethod 
    def round_coordinates_to_6dp(cls, v) -> float: 
        """GPS coordinates beyond 6 decimal places are meaningless (~0.1mm 
precision).""" 
    ...
    def motorcycle_weight_limit(self) -> 'ETARequest': 
        ...
        if self.vehicle_type == 'motorcycle' and self.cargo_weight_kg > 100: 
            raise ValueError( 
                f'Motorcycle cannot carry {self.cargo_weight_kg:.1f}kg (limit: 
100kg). ' 
    ...
    @computed_field 
    @property 
    def is_rush_hour(self) -> bool:  """True if departure is during peak traffic hours.""" 
    ...
    def to_feature_vector(self) -> list[float]: 
        ...
        return [ 
            ...
            float(self.is_rush_hour),                            # feature 2  (1.0 
or 0.0) 
